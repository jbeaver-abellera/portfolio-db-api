...
def lambda_handler(event, context):
    ...
    with conn.cursor() as curr:
        ...
        curr.execute(f"SELECT * FROM {table_name} ORDER BY id DESC LIMIT 1")
        logger.info("The following items have been found in the db.")
        last_row = curr.fetchone() #also the last row
        row_id = last_row[0]
        ...
        curr.execute(f"UPDATE {table_name} SET views={new_count}, timestamp=\'{timestamp}\' WHERE id={row_id}")
